...
int main() {
    ...
    Mat checkboard = imread("checkerboard.png", CV_LOAD_IMAGE_COLOR);
	Mat check2 = imread("checkerboard2.jpg", CV_LOAD_IMAGE_COLOR);
...
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	...
	for (int i = radius; i < row + radius; i++) {
		for (int j = radius; j < col + radius; j++) {
			...
			for (int a = -radius; a <= radius; a++) {// for each kernel window
				for (int b = -radius; b <= radius; b++) {
					if (i + a - radius >= 0 & i + a - radius < row&j + b - radius >= 0 & j + b - radius < col)
						if (input_mirror.at<float>(i + a, j + b) < max)

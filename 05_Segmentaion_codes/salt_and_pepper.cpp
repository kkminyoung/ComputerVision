				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						*/
						...
						kernel.at<G>(index, 0) = input.at<G>(tempx, tempy);
						index++;
					}
				}
			}
...
Mat Salt_pepper_noise_removal_RGB(const Mat input, int n, const char *opt) {
	...
	// ex) CV_8UC3 - CV_8U = 16
	Mat kernel = Mat::zeros(kernel_size * kernel_size, channel, CV_32F);
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			...
			if (!strcmp(opt, "zero-padding")) {
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						*/
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) { //if the pixel is not a border pixel
							kernel.at<G>(index, 0) = input.at<C>(i + x, j + y)[0];
							kernel.at<G>(index, 1) = input.at<C>(i + x, j + y)[1];
							kernel.at<G>(index, 2) = input.at<C>(i + x, j + y)[2];
						}
						index++;
					}
				}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						*/
						...
						kernel.at<G>(index, 0) = input.at<C>(tempx, tempy)[0];
						kernel.at<G>(index, 1) = input.at<C>(tempx, tempy)[1];
						kernel.at<G>(index, 2) = input.at<C>(tempx, tempy)[2];
						index++;
					}
				}
			}
			...
			median = kernel_size * kernel_size / 2 + 1;
			for (int k = 0; k < 3; k++)
				output.at<C>(i, j)[k] = kernel.at<G>(median, k);
		}
	}
	return output;
}
